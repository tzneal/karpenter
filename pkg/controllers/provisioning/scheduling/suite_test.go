...
var _ = Describe("Topology", func() {
	...
	Context("Zonal", func() {
		...
		It("should violate max-skew when unsat = do not schedule", func() {
			topology := []v1.TopologySpreadConstraint{{
				TopologyKey:       v1.LabelTopologyZone,
				WhenUnsatisfiable: v1.DoNotSchedule,
				LabelSelector:     &metav1.LabelSelector{MatchLabels: labels},
		...
		It("should handle a non-self selecting topology", func() {
			topology := []v1.TopologySpreadConstraint{{
	...
	Context("Pod Affinity", func() {
		...
		It("should support pod affinity with hostname topology and hostname topology spread", func() {
			affLabels := map[string]string{"security": "s2"}
			...
			// these pods want to run on different nodes due to topology spread, but also want to run on the same host
			// as affPod1
			affPods := MakePods(10, test.PodOptions{
				ObjectMeta: metav1.ObjectMeta{Labels: labels},
				TopologySpreadConstraints: []v1.TopologySpreadConstraint{
			...
			Expect(nodeNames).To(HaveLen(1))
			// with two pods
			for _, podCount := range nodeNames {
				Expect(podCount).To(Equal(2))
			}
		})
		It("should handle multiple chained affinities", func() {
			dbLabels := map[string]string{"type": "db", "spread": "spread"}
			webLabels := map[string]string{"type": "web", "spread": "spread"}
			cacheLabels := map[string]string{"type": "cache", "spread": "spread"}
